        ...
        <section id="contactSection" class="row mb-5">
            <div class="col-12 col-lg-4">
                ...
                <div class="d-flex align-items-center mb-4">
                    <img src="{{asset('images/contact/location_icon.svg')}}" alt="location icon" id="locationIcon">
            ...
            <form class="col-12 col-lg-8" id="emailForm" action="{{action('App\Http\Controllers\ContactController@store')}}" method="POST">
                @csrf
                @method('POST')
                <div class="mb-3">
                    <label for="fullName" class="mb-2">Full Name <span>&ast;</span></label>
                    <input type="text" id="fullName" class="form-control p-3 @error('name') is-invalid @enderror" placeholder="Full name" name="name" value="{{old('name')}}">
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="email" placeholder="your@email.com" class="form-control @error('email') is-invalid @enderror p-3" id="emailAddress" name="email" value="{{old('email')}}">
                    @error('email')
                    <p class="invalid-feedback">
                        Enter a valid email
                    </p>
                ...
                <div class="mb-3">
                    ...
                    <select name="subject" id="subject" class="form-select p-3 @error('subject') is-invalid @enderror">
                        <option selected disabled>Select a subject</option>
                        <option value="Track Order" @if(old('subject') == 'Track Order') selected @endif>Track Order</option>
                        <option value="Report Issue" @if(old('subject') == 'Report Issue') selected @endif>Report Issue</option>
                        <option value="Return Order @if(old('subject') == 'Return Order') selected @endif">Return Order</option>
                        <option value="Cancel Order" @if(old('subject') == 'Cancel Order') selected @endif>Cancel Order</option>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="message" id="message" class="form-control @error('message') is-invalid @enderror" placeholder="Write your message" rows="5">{{old('message')}}</textarea>
                    @error('message')
                    <p class="invalid-feedback">
                        {{$message}}
                    </p>
